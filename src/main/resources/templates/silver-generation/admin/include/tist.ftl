...
<#macro crudButtons prefix actions=[]>
    <#if actions?seq_contains("add") && checkUrlMatchRole(prefix + "/add")>
    ...
    <#if actions?seq_contains("export") && checkUrlMatchRole(prefix + "/export")>
        <button type="submit" class="btn btn-raised btn-info"
                formaction="<@spring.url "${prefix}/export" />">
            <@spring.message "admin.button.export" />
        </button>
    </#if>
    <#if actions?seq_contains("import") && checkUrlMatchRole(prefix + "/import")>
...
<#macro baseRowButtons updateUrl="" reportUrl="" deleteUrl="" >
    <#if updateUrl != "" && checkUrlMatchRole(updateUrl)>
    ...
    <#if deleteUrl != "" && checkUrlMatchRole(deleteUrl) >
...
<#-- 按鈕 or a標籤 進行權限控管使用，檢查當前url是否符合角色擁有 -->
<#function checkUrlMatchRole(targetUrl) >
    <#assign matchFound = false >
    <#if targetUrl?has_content >
        <#list navMenus as navMenu>
            <#if navMenu.children?has_content>
                <#list navMenu.children as menu>
                    <#if menu?? && targetUrl?contains(menu.url) && menu.actions?has_content>
                        <#list menu.actions as action>
                            <#if action?? && targetUrl?contains(action)>
